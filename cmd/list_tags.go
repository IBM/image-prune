...
`,
		...
		Example: `image-prune list-tags docker://docker.io/fedora`,
	}
	adjustUsage(cmd)
	flags := cmd.Flags()
...
func listFilteredDockerTags(ctx context.Context, sys *types.SystemContext, opts *tagsOptions, repositoryName string, tags []string) (string, []string, error) {
	...
	if opts.filterOpts.BeforeVersion.Present() || opts.filterOpts.BeforeTime.Present() {
		...
		if opts.filterOpts.Invalid.Present() && opts.filterOpts.Invalid.Value() {
			toPrune = append(toPrune, filtered.Invalid...)
		}
		return repositoryName, toPrune, nil
	} else if opts.filterOpts.Invalid.Present() && opts.filterOpts.Invalid.Value() {
